...
const Button: React.FC<ButtonProps> = ({
  ...
  const colorClasses = {
    primary:
      "bg-transparent border-primary-500 border-2  hover:bg-primary-500 focus-visible:outline-primary-600",
    secondary:
  ...
  return (
    <button
      type={type}
      className={`rounded text-white transition-all ${buttonClasses}`}
      onClick={onClick}
      disabled={disabled}
    >
      {isLoading ? (
        ...
        <span className="truncate transition-all">{children}</span>
      )}
    </button>
  );
};
